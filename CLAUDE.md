...
Deploy uses force push to `origin/main`, NOT `hexo deploy`. The target repo is `git@github.com:Jack-infinity420/jack-infinity420.github.io.git`.
...
# 2. Assemble deploy directory (this WIPES public-deploy/ including .git)
node tools/assemble-deploy.js

# 3. Re-init git in public-deploy/ and push
cd public-deploy
git init
git remote add origin git@github.com:Jack-infinity420/jack-infinity420.github.io.git
git add -A
git commit -m "deploy: blog static site update"
git branch -m master main   # git init defaults to master on Windows
git push --force origin main
```

### Key pitfalls

- `assemble-deploy.js` **destroys** `public-deploy/.git` — git must be fully re-initialized after assembly.
- On Windows, `git init` creates `master` by default; the remote expects `main`. Always rename the branch before pushing.
- This is a **force push** — the remote commit history is replaced with the new build output.

`tools/assemble-deploy.js` builds the final directory structure:
```
/                          ← cosmic-intro/index.html (root landing)
/images/cosmic/            ← 3D textures
/blog/                     ← full Hexo blog output
```
